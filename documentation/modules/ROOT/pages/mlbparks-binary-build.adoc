...
[TIP]
====
If you don't have Maven (`mvn`) on your machine but you are able to run docker, you can instead mount the repo directory and run the build commands from within the container.  From within the directory that you ran `git clone` above, run the following command:
...
Then you can run the rest of the commands in this section from within the container
====
